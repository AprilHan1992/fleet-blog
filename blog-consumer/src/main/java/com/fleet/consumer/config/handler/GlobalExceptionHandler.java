...
public class GlobalExceptionHandler {
    ...
    public R handleException(Exception e) {
        if (e != null) {
            if (e instanceof RuntimeException) {
                ...
                if (e instanceof NoHandlerFoundException) {
                    ...
                    return R.error(ResultState.NO_HANDLER_FOUND_ERROR);
                } else if (e instanceof HttpRequestMethodNotSupportedException) {
                    logger.error("不支持的方法：" + e.getMessage(), e);
                    return R.error(ResultState.METHOD_NOT_SUPPORTED_ERROR);
                } else if (e instanceof MissingServletRequestParameterException) {
                    logger.error("缺少参数异常：" + e.getMessage(), e);
                    return R.error(ResultState.PARAM_MISSING);
                } else {
        ...
        return R.error(ResultState.ERROR);
    }
}
